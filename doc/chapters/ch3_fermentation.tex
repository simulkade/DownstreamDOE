...
hopelessly underdetermined fit. Two remedies, both from
Chapter~\ref{ch:stats}, answer this in complementary ways. The first keeps the
linear model but \emph{regularizes} it, so that a wide, correlated, even $p>n$
design becomes well-posed and the few influential strains are selected; this
preserves the prized interpretability of signed coefficients. The second
abandons linearity for methods---tree ensembles---that discover interactions
\emph{implicitly}. We take them in turn.

\subsection{The regularized and generalized linear model}
\label{sec:ferm-glm}

The pathologies above---non-orthogonality, collinearity, more candidate effects
than runs---are exactly the ones that \emph{regularization}
(Section~\ref{sec:stats-reg}) was built to cure. Replacing the least-squares fit
of the final-pH response with an \emph{elastic-net} fit---least squares plus a
blend of the lasso's $L_1$ and the ridge's $L_2$ penalties, the penalty strength
chosen by cross-validation---turns the underdetermined problem into a stable,
sparse one. The $L_1$ part drives most strain coefficients to exactly zero,
returning a short list of the strains that measurably move the pH; the $L_2$ part
keeps the fit graceful under the correlated columns the covering array produces.
The package exposes this as \code{regularized\_importance}, which reports the same
cross-validated $R^2$ and permutation importance as the tree analyses for a fair
comparison, plus the \emph{signed} coefficients the trees cannot give.

Often, though, the question the technologist actually asks is binary: \emph{will
this blend set at all?} The set time is \emph{censored}---a weak blend never
reaches pH~$4.6$ within the window---so rather than discard those runs we recode
the outcome as a yes/no and reach for the \emph{generalized linear model} of
Section~\ref{sec:stats-glm}. A \emph{regularized logistic regression}
(\code{logistic\_screening}) models the probability that a blend sets as a
logistic function of which strains it contains; the $L_1$ penalty again keeps the
selection sparse, and the fit is graded out-of-sample by the area under the ROC
curve. Its positive coefficients name the strains that make setting likely---in
practice the acid-tolerant drivers like \emph{L.\ bulgaricus}---and its negative
ones the strains that, dominating a blend, stall it above the set point.

Figure~\ref{fig:ferm-reg} shows both at work on the same $200$-run screen. The
elastic-net model reaches a cross-validated $R^2\approx0.55$ on final pH---on par
with the tree ensembles below---and its coefficients track each strain's true,
unseen solo acidifying power ($r\approx0.7$); the logistic GLM predicts ``did it
set?'' with an ROC-AUC near $0.8$. The regularized linear model is thus a strong,
interpretable first pass. Its limitation is the same one that motivated the whole
section: with only the main-effect columns it still cannot \emph{discover}
interactions, and enumerating all $1225$ pairwise columns---even regularized---asks
a great deal of $200$ runs. That is the cue for the trees.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{ferm_regularized.png}
\caption[Regularized linear and logistic analysis of the screen]{Classical,
interpretable analyses of the same $200$-run screen.
\emph{Left:} the elastic-net (regularized linear) model of final pH; each
strain's fitted effect tracks its \emph{true} solo acidifying power, which the
screen never measured directly (CV $R^2$ and correlation $r$ in the title).
\emph{Right:} a regularized logistic regression---a generalized linear model---for
the binary outcome ``did the blend reach the set point?''; bars are the strains
whose presence most raises (green) or lowers (red) the probability of setting,
with the cross-validated ROC-AUC and accuracy in the title. Generated by
\code{regularized\_importance} and \code{logistic\_screening}.}
\label{fig:ferm-reg}
\end{figure}
